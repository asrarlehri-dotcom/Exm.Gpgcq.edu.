    ...
    if (res.ok) {
      ...
      data.forEach(c => {
        if (!map[c.cnic] || c.status === "PAID") {
          map[c.cnic] = { cnic: c.cnic, status: c.status, challanNumber: c.challanNumber, amount: c.amount, paidId: c.paidId };
        }
      });
      setChallanMap(map);
    }
              ...
              <tbody>
                {filtered.map((a, i) => (
                  <tr key={a.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      {/* Fee status badge (matched by CNIC) */}
                      {challanMap[a.cnic] ? (
                        <div>
                          ...
                          <div className="text-[10px] text-gray-400 mt-0.5">{challanMap[a.cnic].challanNumber}</div>
