...
export default function BSTimetableDatesheetPage() {
  ...
  const [tSemester, setTSemester] = useState("1");
  ...
  const loadTimetable = async () => {
    ...
    const data = await getTimetables(
      ...
      tSemester ? parseInt(tSemester) : undefined,
      tDepartmentId || undefined
    );
    setTimetables(data.filter((t: any) => t.program?.educationLevel === "BS"));
    setLoading(false);
  };
  ...
  const handleAutoGenerateTimetable = async () => {
    ...
    const sem = tSemester ? parseInt(tSemester) : 1;
    ...
    setError("");
    setSuccess("");
    const res = await generateTimetable(tSession, progId, deptId, sem);
    if (res.errors && res.errors.length > 0) {
      setError(res.errors.join(" | "));
    } else {
      setSuccess("BS Timetable generated successfully!");
    }
    await loadTimetable();
    setLoading(false);
  };
  ...
  const handlePublishTimetable = async () => {
    ...
    const sem = tSemester ? parseInt(tSemester) : 1;
    await publishTimetable(tSession, tProgramId, sem);
  ...
  const handleAutoGenerateDatesheet = async () => {
    const progId = dProgramId;
    const deptId = dDepartmentId || departments[0]?.id;
    const sem = dSemester ? parseInt(dSemester) : 1;
    ...
    if (res.errors && res.errors.length > 0) {
      ...
      setSuccess("BS Date Sheet generated successfully!");
    }
    await loadDatesheet();
    setLoading(false);
  };
  ...
  const handlePublishDatesheet = async () => {
    if (!dProgramId) return alert("Select a program first.");
    const sem = dSemester ? parseInt(dSemester) : 1;
    await publishDatesheet(dSession, dProgramId, sem, dExamType);
    setSuccess("Date Sheet published!");
    await loadDatesheet();
  };
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-bold transition-all shadow-md hover:shadow-lg"
                >
                  ⚡ Auto-Generate draft
                </button>
                <button
                  ...
                  <table className="w-full text-left border-collapse text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b">
                        ...
                        <th className="p-3 font-semibold text-gray-600">Semester</th>
                    ...
                    <tbody>
                      {timetables.map((t) => (
                        <tr key={t.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3 font-medium">Sem {t.semester}</td>
                          <td className="p-3 font-semibold">{t.dayOfWeek}</td>
                          <td className="p-3">{t.startTime} - {t.endTime}</td>
                          <td className="p-3 font-bold text-gray-800">{t.course?.title}</td>
            ...
            <div className="space-y-4">
              ...
              <div className="overflow-x-auto border rounded-xl mt-4">
                {datesheets.length > 0 ? (
                    ...
                    <tbody>
                      {datesheets.map((d) => (
                        <tr key={d.id} className="border-b hover:bg-gray-50">
                          <td className="p-3 font-semibold">{new Date(d.date).toLocaleDateString()}</td>
                          <td className="p-3">{d.startTime} - {d.endTime}</td>
                          <td className="p-3 font-bold text-gray-800">{d.course?.title}</td>
                          <td className="p-3">
                            <span className="px-2 py-0.5 text-xs font-semibold rounded bg-indigo-50 text-indigo-700 border border-indigo-200">
                              {d.examType === 'FINAL_TERM' ? 'Final' : 'Mid'}
                            </span>
                          </td>
                          <td className="p-3">
                            <span className={`px-2 py-1 text-xs rounded-full font-bold ${d.status === 'PUBLISHED' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                              {d.status}
                            </span>
                          </td>
                          <td className="p-3">
                  ...
                  <p className="text-gray-500 text-center py-8">No Datesheet generated yet.</p>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === "dutylist" && (
            <div className="space-y-4">
              <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-blue-900">🛡️ Staff Invigilation Assignments</h3>
                  <p className="text-xs text-blue-700 mt-1">Duties generated automatically based on exam schedules and faculty availability.</p>
                </div>
                <button
                    ...
                    <tbody>
                      {duties.map((duty) => (
                        <tr key={duty.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3 font-bold text-indigo-900">{duty.invigilator}</td>
                          <td className="p-3">
